...
const KEY_DIR = process.env.KEY_DIR || path.join(CRYPTO_PATH, 'users', 'User1@org1.example.com', 'msp', 'keystore');
const CERT_DIR = process.env.CERT_DIR || path.join(CRYPTO_PATH, 'users', 'User1@org1.example.com', 'msp', 'signcerts');
const TLS_CERT_PATH = process.env.TLS_CERT_PATH || path.join(CRYPTO_PATH, 'peers', 'peer0.org1.example.com', 'tls', 'ca.crt');
...
app.post('/api/assets', async (req, res) => {
    try {
        ...
        await contract.submitTransaction('PublishAsset', assetID, name, description, owner, value.toString());
        res.json({
            success: true,
...
app.put('/api/assets/:assetID', async (req, res) => {
    try {
        ...
        await contract.submitTransaction('UpdateAsset', assetID, name, description, owner, value.toString());
        res.json({
            success: true,
...
app.post('/api/releases', async (req, res) => {
    try {
        ...
        await releaseContract.submitTransaction('PublishRelease', packageId, version, fileHash);
        res.json({
            success: true,
...
app.put('/api/releases/:packageId/:version/discontinue', async (req, res) => {
    try {
        ...
        await releaseContract.submitTransaction('DiscontinueRelease', packageId, version);
        res.json({
            success: true,
...
app.post('/api/releases/validate', async (req, res) => {
    try {
        ...
        const result = await releaseContract.evaluateTransaction('ValidateRelease', packageId, version, fileHash);
...
app.get('/api/releases/:packageId/:version', async (req, res) => {
    try {
        ...
        const { packageId, version } = req.params;
        const result = await releaseContract.evaluateTransaction('GetRelease', packageId, version);
